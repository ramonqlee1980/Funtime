...
-(NSMutableURLRequest *)createURLRequest
{
    ...
    if([m_HttpMethod isEqualToString:@"GET"])
    {
        ...
        for(NSInteger index=0; index < [[m_ParamDic allKeys] count]; index++)
        {
            ...
            if(index == 0)
                [urlFullText appendFormat:@"?%@=%d",key,MobiSageUrlEncodedString([m_ParamDic objectForKey:key])];
            else
                [urlFullText appendFormat:@"&%@=%d",key,MobiSageUrlEncodedString([m_ParamDic objectForKey:key])];
        }
...
{    
    ...
    for(NSInteger index=0; index < [[m_ParamDic allKeys] count]; index++)
    {
        ...
        if([queryText length] == 0)
            [queryText appendFormat:@"%@=%d",key,MobiSageUrlEncodedString([m_ParamDic objectForKey:key])];
        else
